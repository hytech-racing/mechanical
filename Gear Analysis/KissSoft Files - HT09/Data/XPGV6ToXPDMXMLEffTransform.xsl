...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="cfg:CfgEffectivityExpression">
		<xsl:choose>
			<!-- using this condition we stop the export of effectivity expression 
	...
	<xsl:template match="cfg:Feature">
		<xsl:element name="FeatureCondition">
	...
	<xsl:template match="cfg:Series">
		<xsl:element name="UnitCondition">
			<xsl:for-each select="*">
				<xsl:apply-templates select="." />
			</xsl:for-each>
		</xsl:element>
	...
	<xsl:template match="cfg:Date">
		<xsl:element name="DateCondition">
			<xsl:for-each select="*">
				<xsl:apply-templates select="." />
			</xsl:for-each>
		</xsl:element>
	...
	<xsl:template match="cfg:TreeSeries">
		<xsl:choose>
			<xsl:when test="@Type='ProductState'">
				<xsl:element name="ProductStateCondition">
					<xsl:apply-templates select="*" />
				</xsl:element>
			</xsl:when>
			<xsl:when test="@Type='ManufPlan'">
				<xsl:element name="ManufacturingPlanCondition">
					<xsl:apply-templates select="*" />
				</xsl:element>
			</xsl:when>
		</xsl:choose>
	...
	<!-- template MilestoneCondition -->
	<xsl:template match="cfg:MilestonesTrack">
		<xsl:element name="MilestoneCondition">
