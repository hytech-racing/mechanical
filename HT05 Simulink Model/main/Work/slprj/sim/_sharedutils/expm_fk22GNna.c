...
#include <math.h>
#include "PadeApproximantOfDegree_Whgw9LNw.h"
#include "mwmathutil.h"
#include "expm_fk22GNna.h"

void expm_fk22GNna(real_T A[4], real_T F[4])
{
  real_T normA;
  real_T b_s;
  int32_T b_j;
  ...
  if (normA <= 5.3719203511481517) {
    ...
    while ((!exitg1) && (b_j < 5)) {
      if (normA <= theta[b_j]) {
        PadeApproximantOfDegree_Whgw9LNw(A, b[b_j], F);
        exitg1 = true;
      } else {
        b_j++;
      }
    ...
    A[2] /= b_s;
    A[3] /= b_s;
    PadeApproximantOfDegree_Whgw9LNw(A, 13, F);
